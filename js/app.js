/**
 * app.js — Logika & tampilan aplikasi Jadwal Pelajaran SMK Negeri 1 Bitung.
 * File ini TIDAK berisi data yang perlu diedit rutin — semua data ada di
 * file terpisah yang dimuat sebelumnya (lihat index.html untuk urutannya):
 *   - js/data/subjects.js  nama & warna tiap mata pelajaran
 *   - js/data/time.js      jam masuk & durasi tiap jenis kegiatan
 *   - js/data/majors.js    daftar jurusan per tingkat kelas
 *   - js/data/schedule.js  jadwal per kelas/jurusan/rombel/hari
 *   - js/data/funfacts.js  isi & ilustrasi halaman Fun Facts
 *   - js/uniform.js        konfigurasi & gambar seragam harian
 * Kalau mau mengubah DATA (nama mapel, jam, jadwal, fun facts, seragam),
 * edit file-file di atas, bukan file ini.
 */
...
/* ===================================================================
   2. WARNA & NAMA MATA PELAJARAN
   Dipindah ke js/data/subjects.js — cari getSubjectName() &
   getSubjectColorVar() di sana. Diletakkan di file data terpisah supaya
   nama mapel yang salah/kepanjangan bisa dibetulkan di SATU tempat saja.
   =================================================================== */
...
function renderSchedule(day) {
  ...
  // scheduleData[...][day] sekarang hanya daftar KODE mapel berurutan
  // (tanpa jam). Jamnya dihitung otomatis di sini dari js/data/time.js —
  // ubah durasi/jam masuk di sana, bukan di file jadwal.
  const subjectCodes = scheduleData[selectedClass] && scheduleData[selectedClass][selectedMajor] && scheduleData[selectedClass][selectedMajor][selectedRombel]
    ? scheduleData[selectedClass][selectedMajor][selectedRombel][day]
    : null;
  const schedule = subjectCodes ? hitungJadwalHari(subjectCodes, day) : null;
  ...
  if (schedule && schedule.length) {
    ...
    schedule.forEach((item, i) => {
      ...
      // item.subject adalah KODE mapel (mis. "MATEMATIKA"), bukan nama
      // lengkap — nama & warna sebenarnya diambil dari js/data/subjects.js.
      const subjectCode = item.subject || '';
      const subjectName = getSubjectName(subjectCode);
      const colorVar = getSubjectColorVar(subjectCode);
      row.innerHTML = `
...
function renderFunFacts() {
  ...
  funFactsData.forEach((fact, i) => {
    ...
    // "accent" & "icon" datang langsung dari kartu di js/data/funfacts.js
    // (bukan array terpisah berbasis index), supaya aman kalau kartu
    // ditambah/dihapus/diurutkan ulang di file data tersebut.
    card.style.setProperty('--fact-accent', `var(${fact.accent || '--accent'})`);
    ...
    card.innerHTML = `
      <div class="fact-card-head">
        <div class="fact-illustration">${FACT_ICONS[fact.icon] || ''}</div>
